...
class SleepRepository(BaseRepository[Sleep]):
    ...
    async def get_by_date(self, user_id: int, entry_date: date) -> Sleep | None:
        start_dt = datetime.combine(entry_date, datetime.min.time(), tzinfo=timezone.utc)
        end_dt = datetime.combine(entry_date + timedelta(days=1), datetime.min.time(), tzinfo=timezone.utc)
        stmt = select(self.model).where(
            self.model.user_id == user_id,
            self.model.bed_time >= start_dt,
            self.model.bed_time < end_dt,
        )
